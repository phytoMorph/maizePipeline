function [T ufT BB PS MT sM] = measureKernelLength_forcondor(I,numberCobs,RAD,gridSites)
    ufT = 0;
    PS = [];
    PS.widthProfile = [];
    sM = {};
    try        
        ...
        for e = 1:numberCobs
            ...
            subI = R(e).Image;
            grayImage = double(imcrop(I,R(e).BoundingBox))/255;
            ...
            parfor r = 1:numel(RAD)
                ...
                MT(r,:,e) = [Tg Tl];
                T(e,r) = nanmean([Tg Tl]);                
                fprintf(['done with window ' num2str(r) ':' num2str(numel(RAD)) '\n']);
            end
            
            fprintf(['done with ear ' num2str(e) ':' num2str(numberCobs) '\n']);
            ufT.G{e} = tG;
            ufT.L{e} = tL;
        end
    catch ME
        getReport(ME);
        fprintf(['******measure kernel length******\n']);
    end
end
